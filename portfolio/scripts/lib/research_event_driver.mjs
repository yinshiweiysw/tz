...
const THEMES = [
  {
    key: "geopolitics_ceasefire",
    label: "中东停火预期重估全球风险偏好",
    keywords: ["停火", "ceasefire", "伊朗", "中东", "缓和", "风险偏好"]
  },
  {
    key: "geopolitics_oil",
    label: "中东地缘升级推动油价再定价",
...
const THEME_METADATA = {
  geopolitics_ceasefire: {
    driver_type: "geopolitics",
    expected_consensus: "市场原本预期停火更多停留在口头阶段，不足以立刻重塑风险偏好。",
    positiveConfirmationHint: "若权益反弹、美元回落而黄金仍偏强，说明停火并未完全消除宏观不确定性。",
    negativeConfirmationHint: "若仅 headlines 升温但跨资产未确认，则仍停留在观察阶段。"
  },
  geopolitics_oil: {
    driver_type: "geopolitics",
...
function normalizeStory(item = {}) {
  ...
  const tier = Number(item?.tier ?? 3);
  const parsedTime = new Date(item?.published_at ?? item?.publishedAt ?? "");
  return {
    headline,
    score: scoreHeadline(headline) + (tier === 1 ? 5 : tier === 2 ? 3 : 1),
    ...classifyTheme(headline),
    published_at: Number.isFinite(parsedTime.getTime()) ? parsedTime.toISOString() : null,
...
export function buildResearchEventDriver({ stories = [], telegraphs = [], marketSnapshot = {} } = {}) {
  const storyInput = Array.isArray(stories) && stories.length > 0 ? stories : telegraphs;
  const normalized = storyInput
    .map((item) => ({
      ...normalizeStory(item)
    }))
    .filter((item) => item.headline.length > 0);

  const corroborationCount = normalized.reduce((map, item) => {
    const key = item?.theme?.key ?? item.headline;
    map.set(key, (map.get(key) ?? 0) + 1);
    return map;
  }, new Map());
  for (const item of normalized) {
    const corroboration = corroborationCount.get(item?.theme?.key ?? item.headline) ?? 1;
    item.score += Math.max(corroboration - 1, 0) * 2;
  }
